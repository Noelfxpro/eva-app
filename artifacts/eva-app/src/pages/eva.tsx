...
async function verifyEd25519(signature: string, message: string, publicKey: string): Promise<boolean> {
  try {
    ...
    return await crypto.subtle.verify(
      'Ed25519', cryptoKey, hexToBytes(signature), new TextEncoder().encode(message)
    )
  } catch { return false }
}
...
export default function EVA() {
  ...
  // Reset crypto state whenever hash changes (content was edited after signing)
  useEffect(() => {
  ...
  const anchorOnAptos = async () => {
    ...
    setAnchoring(true)
    try {
      // Encode hash string as hex bytes for the vector<u8> argument
      ...
      alert('Anchoring failed: ' + (err instanceof Error ? err.message : String(err)))
    } finally { setAnchoring(false) }
  }

  const handlePublish = () => {
    if (!author || !title || !body || !hash) {
      alert('Please fill in all fields and generate a hash first.'); return
    }
  ...
  const inp: React.CSSProperties = { padding: 8, fontSize: 14 }
    ...
    <div style={{ padding: 30, fontFamily: 'Arial', maxWidth: 640 }}>
      ...
      {tab === 'feed' && (
        <div>
          ...
          {feedQuery.isLoading && <p style={{ color: '#6b7280' }}>Loading…</p>}
          {feedQuery.isError && <p style={{ color: 'red' }}>Error loading feed.</p>}
          {feedQuery.data?.posts.length === 0 && <p style={{ color: '#6b7280' }}>No posts yet.</p>}
